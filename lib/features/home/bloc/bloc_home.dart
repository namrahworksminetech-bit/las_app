...
class HomeBloc extends Bloc<DashboardEvent, DashboardState> {
  HomeBloc() : super(DashboardState(routeList: [EmPage.dashboard])) {
    on<OnClickBack>(_onBackEvent);
    on<OnClickTab>(_onTabEvent);
  }

  _onBackEvent(OnClickBack event, Emitter<DashboardState> emit) {
  ...
  _onTabEvent(OnClickTab event, Emitter<DashboardState> emit) {
